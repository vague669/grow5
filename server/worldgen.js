...
function newWorldTiles() {
  ...
  for (let y = 0; y < WORLD_H; y++) {
    ...
    for (let x = 0; x < WORLD_W; x++) {
      let id = null;
      if (y >= WORLD_H - 9) {
        id = (y === WORLD_H - 9) ? 'grassdirt' : (y > WORLD_H - 4 ? 'cave' : 'dirt');
      }
...
const DOOR_TX = Math.floor(WORLD_W/2), DOOR_TY = WORLD_H-10;
