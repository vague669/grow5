...
const { liveWorlds, tradeSessions, onlineUsers, mutedUsers, missionSessions, getIO } = require('./state');
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('auth:join', async ({ token, worldName }, ack) => {
      try {
        ...
        const spawnX = live.isMatrix ? live.mainSpawn.x * 32 : 30 * 32;
        const spawnY = live.isMatrix ? live.mainSpawn.y * 32 : 5 * 32;
    ...
    socket.on('item:openpack', async ({ itemId }, ack) => {
      if (!ctx) return;
      if (itemId !== 'seedpack') return ack && ack({ error: 'Bu eşya açılamaz.' });
      const userRes = await pool.query('SELECT inventory FROM users WHERE id=$1', [ctx.uid]);
      const inventory = userRes.rows[0].inventory;
      if (!invRemove(inventory, 'seedpack', 1)) return ack && ack({ error: 'Envanterinde Seed Pack yok.' });
      SEEDPACK_TABLE.forEach(entry => invAdd(inventory, entry.id, entry.count));
      await persistUserRow(ctx.uid, { inventory });
      ack && ack({ ok: true, inventory, opened: SEEDPACK_TABLE });
    });
